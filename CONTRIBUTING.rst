...
Water could always use more documentation, whether as part of the
official Water docs, in docstrings, or even on the web in blog posts,
articles, and such.
...
Ready to contribute? Here's how to set up `Water` for local development.

1. Fork the `Water` repo on GitHub.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/water.git
...
2. The tests that cover a module called ``water/path/to/a_module.py``
   should be implemented in a separated module called
   ``tests/water/path/to/test_a_module.py``.
...
To run a subset of tests::
    ...
    $ python -m pytest tests.test_water
...
2. Update the version in ``setup.cfg``, ``water/__init__.py`` and
   ``HISTORY.md`` files.
3. Create a new git tag pointing at the corresponding commit in ``stable`` branch.
4. Merge the new commit from ``stable`` into ``master``.
5. Update the version in ``setup.cfg`` and ``water/__init__.py``
...
.. _GitHub issues page: https://github.com/D3-AI/water/issues
.. _Travis Build Status page: https://travis-ci.org/D3-AI/water/pull_requests
